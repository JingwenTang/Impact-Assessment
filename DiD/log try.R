...
for (tm in c(1,2)){
  ...
  for (p in 1:length(Product)){
    ...
    marketdat$month  <- as.Date(cut(marketdat$date,
                              breaks = "month"))
    newd = join(marketdat,WPI,by = "month",type = "left")
    newd$wpi = as.numeric(as.character(newd$wpi))
    newd$modal = as.numeric(as.character(newd$modal))
    newd$nowpimodal = (newd$modal)*(newd$wpi)
    newd$nowpimodal = newd$nowpimodal/100
    marketdat = newd[,-(ncol(newd)-2)]
    ...
    try({
      ...
      fit2 = lm(log(nowpimodal)~ market+date+annual+total_ar+lit_ru+ptmrkt+agrl_t+
                  Per.Capita.GSDP+production+yield+wpi+
                  log(amount)+ rainfall+rainfall_lead1+rainfall_lead2+rainfall_lead3+rainfall_lead4
                +rainfall_lead5+rainfall_lead6+rainfall_lead7+rainfall_lead8+rainfall_lead9
                +rainfall_lead10+rainfall_lead11+rainfall_lead12+inter,
                data=d)
    ...
    write.csv(crse1,paste(product,"log modal.csv"))
    write.csv(crse2,paste(product,"log no wpi modal.csv"))
    
    
    a1 = stargazer(crse1)
    a2 = stargazer(crse2)
   
    write(a1,paste(product,"log modal.txt"))
    write(a2,paste(product,"log no wpi modal.txt"))
    ...
    trial = c("log modal","log no wpi modal")
  ...
  saveWorkbook(wbe,paste("/Users/tangjingwen/Dropbox/jingwen_tasks/organized/result/DiD/",timemode," inter effects log.xlsx",sep = ""),overwrite = T)
  saveWorkbook(wb,paste("/Users/tangjingwen/Dropbox/jingwen_tasks/organized/result/DiD/",timemode," inter coefficients log.xlsx",sep = ""),overwrite = T)
}
