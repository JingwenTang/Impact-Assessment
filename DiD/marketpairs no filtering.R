...
for (tm in c(6)){
  ...
  for (p in 1:length(Product)){
    try({
      ...
      pairdat$dist = as.numeric(pairdat$dist)
      pairdat = pairdat[order(pairdat$dist,decreasing = T),]
  ...
  timemode = Timemode[tm]
    ...
    for (p in 1:length(Product)){
      ...
      try({
        ...
        marketname90 = amount$market
        marketname90 = unique(marketname90)
        newmarketname90 = marketname90[marketname90%in%overall]
        num90 = length(newmarketname90)
        top5 = newmarketname90[newmarketname90%in%intervented]
        top5 = top5[1:5]
        ...
        for (l in 1:nrow(dat)){
          ...
          tempname1 = temp[[1]][1]
          tempname2 = temp[[1]][2]
          if ((tempname1%in%top5)&(tempname2%in%top5)){
            newdat5 = rbind(newdat5,dat[l,])
          }
        }
        newdat5$commodity = product
        aver5 = mean(newdat5$dist)
        
      })
      
      
      dat5 = rbind(dat5,newdat5)
      cal = rbind(cal,c(overallnum,num90,aver5,aver))
    }
    colnames(cal) = c("overall markets","number of markets in top 90%","average distance of top 5 markets","average distance")
    cal = cbind(product = Product,cal)

    write.xlsx(dat,paste("/Users/tangjingwen/Dropbox/jingwen_tasks/organized/result/marketpairs/",timemode," all market pairs no filtering.xlsx",sep = ""),overwrite = T)
    write.xlsx(dat5,paste("/Users/tangjingwen/Dropbox/jingwen_tasks/organized/result/marketpairs/",timemode," market pairs no filtering top 5.xlsx",sep = ""),overwrite = T)
    
  write.xlsx(cal,paste("/Users/tangjingwen/Dropbox/jingwen_tasks/organized/result/marketpairs/",timemode," data output no filtering.xlsx",sep = ""),overwrite = T)
