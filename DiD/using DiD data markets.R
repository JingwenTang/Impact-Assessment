...
colnames(Dat)[colnames(Dat)=="mean"]="modal"
use = Dat[Dat$market=="karnataka",]
dat = dat[dat$state!="karnataka",]
dat = dat[,colnames(dat)[colnames(dat)%in%colnames(Dat)]]
Dat = rbind(use,dat)
#balance the panel
...
dataprep.out <-
  dataprep(foo = d,
           predictors = colnames(d)[c(4,11:13,26)] ,
           predictors.op = "mean" ,
...
path.plot(synth.res = synth.out,
          ...
          Ylab = "modal price",
          Xlab = "13-17 month",
          Legend = c(as.character("karnataka"),paste("synthetic",as.character("karnataka"),sep = " ")),
