...
for (size in 3:3){
  ...
  for (p in 1:length(Product)){
    ...
    for (m in 3:3){
      ...
      #remove around states
      dat = dat[!dat$state%in%c("goa","andhra pradesh","tamil nadu","maharashtra","kelala"),]
      ...
      for (j in 1:(length(subintervented))){
        ...
        try({
          ...
          dataprep.out <-
            dataprep(foo = d,
                     predictors = colnames(d)[c(3,6:(6+((length(predictors)+5)+floor((subintervention-1)/setpre[m]))))] ,
                     predictors.op = "mean" ,
