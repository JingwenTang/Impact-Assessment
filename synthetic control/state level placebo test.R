...
for (size in 3:3){
  ...
  Model = c("08-17 mon-mon","08-17 mon-qua","08-17 qua-qua","08-17 qua-half")
  Begin = c(2008,2008,2008,2008)
  Over = c(2017,2017,2017,2017)
  Br = c("month","month","3 months","3 months")
  early = as.Date(c("2008-01-01","2008-01-01","2008-01-01","2008-01-01"))
  ...
  for (p in 5:length(Product)){
    ...
    for (m in 3:3){
      ...
      MS = ms
      ms = ms[,c(1,4)]
      statelist = read.xlsx("/Users/tangjingwen/Dropbox/jingwen_tasks/SCM/statelist.xlsx",colNames = F)$X1
      ms = ms[ms$state%in%statelist,]
      ...
      temp = dat
      Dat = aggregate(temp$amount,by = list(temp$month,temp$state),mean,na.rm=T)
      colnames(Dat) = c("month","state","amount")
      for (l in c(4:6)){
        name = colnames(temp)[l]
        colnames(temp)[l] = "target"
        newdat = as.data.frame(ddply(temp,.(month,state),summarise, wmean = weighted.mean(target,amount,na.rm = T)))
        colnames(newdat) = c("month","state",name)
        Dat = join(Dat,newdat,by = c("month","state"),type = "left")
        colnames(temp)[l] = name
        
      }
      dat = Dat
      colnames(dat)[2] = "market"
      subintervented = "karnataka"
      a = dat
      a1 = a[a$market=="karnataka",]
      a2 = a[!a$market=="karnataka",]
      
      Sigma_reci = c()
      Sigma_reci2 = c()
      Gaps = list()
      success = c()
      for (j in 1:(length(subintervented))){
        ...
        try({
          ...
          tempthres = sum(temp$amount,na.rm = T)*0.8
          ...
          setwd("/Users/tangjingwen/Dropbox/jingwen_tasks/organized/result/synthetic control/state level")
          inter_date = c()
          ...
          inter_date = inter_date[intervention$X3%in%dat6$market]
          temp = min(inter_date)
          
          interinfo =data.frame(market = "karnataka",inter_date = inter_date)
          subinter = data.frame(market = "karnataka")
          ...
          pdf(paste("path plot5 of", product,Model[m],".pdf",sep = " "),8,4)
          path.plot(synth.res = synth.out,
                    ...
                    Main = paste(subintervented[i]," of ",product)
          )
          abline(v=subintervention,lty = "dotted",lwd = 1.5)
          dev.off()
          pdf(paste("gap plot5 of", product,Model[m],".pdf",sep = " "),8,4)
      ...
      saveWorkbook(data_details, paste(product," data_details1.xlsx",sep = ""), overwrite = TRUE)
    }#a model ends
    ...
    saveWorkbook(wb, paste(product,"result tables1.xlsx",sep = ""), overwrite = TRUE)
    
    try({
      #placebo tests
      placeboratio = RMSPE2/RMSPE1
      
      for (l in 1:length(nonin)){
        #n0 = d[d$market==unmar[l],"marketn0"][1]
        nonins = nonin[-l]
        dataprep.out <-
          dataprep(foo = d,
                   predictors = colnames(d)[c(7:((length(predictors)+5)+floor((subintervention-1)/setpre[m])))] ,
                   ...
                   unit.names.variable = "market",
                   time.variable = "Month",
                   treatment.identifier = nonin[l],
                   controls.identifier = nonins,
                   time.optimize.ssr = (min(d$Month)):(subintervention-1),
                   time.plot = (min(d$Month)):(max(d$Month))
          )
        synth.out <- synth(data.prep.obj = dataprep.out,method = "BFGS")
        ...
        synth.tables <- synth.tab(dataprep.res = dataprep.out,
                                  synth.res = synth.out)
        placeboratio = c(placeboratio,RMSPE2/RMSPE1) 
        
      }
      treat = c("treated",rep("untreated",length(nonin)))
      dat = data.frame(market = c("karnataka",nonin),ratio = placeboratio,treat = treat)
      qt = 1-ecdf(dat$ratio)(dat[dat$market=="karnataka","ratio"])
      setwd("/Users/tangjingwen/Dropbox/jingwen_tasks/organized/result/synthetic control/state level/placebo")
      pdf(paste(Product[p]," placebo dot chart.pdf",sep = ""), width=8, height=4)
...
####dealing with the state names of market district states
ms = read.xlsx("/Users/tangjingwen/Dropbox/jingwen_tasks/SCM/market_districts.xlsx",sheet=1)
...
ms$dis1 = tolower(ms$dis1)
ms$dis2 = tolower(ms$dis2)
ms$country = tolower(ms$country)
ms$X6 = tolower(ms$X6)
ms$X7 = tolower(ms$X7)
ms$X8 = tolower(ms$X8)
ms$X9 = tolower(ms$X9)
ms$X10 = tolower(ms$X10)
ms$X11 = tolower(ms$X11)
ms$X12 = tolower(ms$X12)
ms[is.na(ms$state),"state"] = ms$dis2[is.na(ms$state)]
ms[ms$state=="india","state"] = ms$dis2[ms$state=="india"]
ms[(ms$state=="india"|is.na(ms$state)),"state"] = ms$dis1[ms$state=="india"|is.na(ms$state)]

a = table(ms$state)[table(ms$state)>=3]
statelist = rownames(a)[!rownames(a)%in%c("chennai","hyderabad","secunderabad")]
statelist = c(statelist,"arunachal pradesh","jammu and kashmir","mizoram","sikkim")
unique(ms$state)
for (l in 1:nrow(ms)){
  if (!ms[l,"state"]%in%statelist){
    
    if (sum(ms[l,]%in%statelist,na.rm = T)>0)  {
      ms[l,"state"] = ms[l,][ms[l,]%in%statelist][1]
    }
  }
}
unique(ms$state)
#write.xlsx(ms,"/Users/tangjingwen/Dropbox/jingwen_tasks/SCM/new market_districts.xlsx")